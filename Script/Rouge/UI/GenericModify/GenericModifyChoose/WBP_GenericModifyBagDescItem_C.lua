...
function WBP_GenericModifyBagDescItem_C:InitGenericModifyDescItem(GenericModifyLevelId, GenericModifyId, bIsUpgrade, ModifyLevelDescShowType, bIsShowHelpInUI)
  ...
  if ModifyLevelDescShowType == UE.EModifyLevelDesc.Addition then
    ...
    if bIsUpgrade and Level2DataMap then
      local ParamCur = Level2DataMap:Find(Level + 1)
      if ParamCur then
  ...
  elseif ModifyLevelDescShowType == UE.EModifyLevelDesc.FinalValue then
    ...
    if bIsUpgrade then
      local nextValue = UE.URGGenericModifyComponent.GetGenericModifyBaseDamageFormActor(Character, GenericModifyId, Level + 1)
      if IsInterger(nextValue) then
