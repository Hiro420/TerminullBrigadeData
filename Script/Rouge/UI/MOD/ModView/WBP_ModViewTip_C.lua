local WBP_ModViewTip_C = UnLua.Class()
function WBP_ModViewTip_C:InitModTipInfo(ModInfo, CurrentModLevel, ModLevelList)
  if not ModInfo then
  ...
  local widgetClass = UE.UClass.Load("/Game/Rouge/UI/MOD/ModView/WBP_ModLevelDes.WBP_ModLevelDes_C")
  local widget
  local Count = 0
  for key, value in pairs(ModLevelList) do
    if key > 1 then
      ...
      if widget then
        widget:InitLevelDes(CurrentModLevel, key, value)
