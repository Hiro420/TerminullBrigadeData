...
function BP_NPC_UpgradeModifyBase_C:ReceiveBeginPlay()
  ...
  print("BP_NPC_UpgradeModifyBase_C:ReceiveBeginPlay bIsEditorOnlyActor", self, self.bIsEditorOnlyActor)
  ...
  if self.RGInteractComponent_UpgradeModify then
    self.RGInteractComponent_UpgradeModify.OnActorFinishInteract:Add(self, self.FinishInteract)
    self.RGInteractComponent_UpgradeModify.StatusChangeDelegate:Remove(self, self.StatusChanged)
...
function BP_NPC_UpgradeModifyBase_C:ReceiveEndPlay(EndPlayReason)
  ...
  print("BP_NPC_UpgradeModifyBase_C:ReceiveEndPlay bIsEditorOnlyActor", self, self.bIsEditorOnlyActor)
  if self.bIsEditorOnlyActor then
    return
  end
  local UIManager = UE.USubsystemBlueprintLibrary.GetGameInstanceSubsystem(GameInstance, UE.URGUIManager:StaticClass())
  ...
  self:NPCAppear(false)
  if self.RGInteractComponent_UpgradeModify then
    self.RGInteractComponent_UpgradeModify.OnActorFinishInteract:Remove(self, self.FinishInteract)
    self.RGInteractComponent_UpgradeModify.StatusChangeDelegate:Remove(self, self.StatusChanged)
  end
  local Character = UE.UGameplayStatics.GetPlayerCharacter(GameInstance, 0)
  if Character then
