...
local safe_pairs = function(t)
  local _next, _t, _nil = pairs(t)
  local safe_next = function(_t, i)
...
local load_error_handler = function(err)
  local msg = err .. "\n" .. debug.traceback()
  UnLua.LogError(msg)
end
local call_hook = function(name, ...)
...
local get_last_modified_time = function(module_name)
  ...
  local require = function(module_name, ...)
    ...
    if func then
      ...
      if nil == loaded_modules[module_name] then
        loaded_modules[module_name] = new_module or true
        package.loaded[module_name] = new_module or true
...
local merge_objects = function(module_res)
...
local collect_module_info = function(module)
  local ret = {}
...
local match_module = function(new_module_info, old_module)
  local ret = {}
...
local match_upvalues = function(value_info_map, old_upvalues)
  local ret = {}
...
local update_global = function(value_map)
...
local update_modules = function(old_modules, new_modules, new_envs)
