...
function PrivilegeData:GetResIdByPrivilegeId(PrivilegeId)
  ...
  if PrivilegeTable then
    for ResId, PrivilegeInfo in pairs(PrivilegeTable) do
      for i, id in ipairs(PrivilegeInfo.PrivilegeID) do
        if id == tonumber(PrivilegeId) then
          return ResId
        end
      end
    end
...
function PrivilegeData:GetMaxPrivilegeQuality(UserId)
  ...
  if MonthCardTable then
    for PrivilegeId, PrivilegeInfo in pairs(PrivilegeData.PrivilegeRoleInfo[UserId]) do
